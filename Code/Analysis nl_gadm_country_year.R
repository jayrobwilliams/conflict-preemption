...
gadm <- gadm %>% mutate(state_ind = as.numeric(as.factor(gwid)),
...
mod_int_pop <- brm(brmsformula(nl ~ cap_dist + pop_tot + cap_dist:pop_tot +
                               ...
                               center = F),
                   data = gadm_log, family = gaussian(), prior = mod_priors,
                   stanvars = mod_stanvars,
                   iter = 4000, chains = 4, save_dso = T,
                   control = list(adapt_delta = .95), seed = 1234,
                   future = T, file = here::here('Stanfits/pd_lm_int_pop_gadm_cy'))
...
mod_int_bord <- brm(brmsformula(nl ~ cap_dist + border + cap_dist:border +
                    ...
                    iter = 4000, chains = 4, save_dso = T,
                    control = list(adapt_delta = .95), seed = 1234,
                    future = T, file = here::here('Stanfits/pd_lm_int_bord_gadm_cy'))
...
margs_interactive_pop_gadm <- mcmcMargEff(mod_int_pop, 'b_pop_tot', 'b_cap_dist:pop_tot',
                                          gadm_log$cap_dist, plot = F)

## write marginal effects plot dataframe to disk
save(margs_interactive_pop_gadm, file = here::here('Figure Data/marg_eff_pop_df_gadm_cy.RData'))

## get marginal effects for interactive models
margs_interactive_bord_gadm <- mcmcMargEff(mod_int_bord, 'b_borderTRUE', 'b_cap_dist:borderTRUE',
                                           gadm_log$cap_dist, plot = F)

## write marginal effects plot dataframe to disk
save(margs_interactive_bord_gadm, file = here::here('Figure Data/marg_eff_bord_df_gadm_cy.RData'))

## print script to verify successful execution in log
print(paste('Nightlights GADM Analysis Completed', Sys.time()))

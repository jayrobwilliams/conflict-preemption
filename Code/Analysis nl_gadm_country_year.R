...
mod_int_pop <- brm(brmsformula(nl ~ cap_dist + pop_tot + cap_dist:pop_tot +
                   ...
                   seed = 1234, future = T,
                   file = here::here('Stanfits/pd_lm_int_pop_gadm_cy'))
...
mod_int_bord <- brm(brmsformula(nl ~ cap_dist + border + cap_dist:border +
                    ...
                    save_dso = T, control = list(adapt_delta = .95),
                    seed = 1234, future = T,
                    file = here::here('Stanfits/pd_lm_int_bord_gadm_cy'))
...
margs_interactive_pop_gadm <- mcmcMargEff(mod_int_pop, 'b_pop_tot',
                                          'b_cap_dist:pop_tot',
                                          scale(gadm_log$cap_dist,
                                                center = T, scale = F),
                                          plot = F)

## write marginal effects plot dataframe to disk
save(margs_interactive_pop_gadm,
     file = here::here('Figure Data/marg_eff_pop_df_gadm_cy.RData'))

## get marginal effects for interactive models
margs_interactive_bord_gadm <- mcmcMargEff(mod_int_bord, 'b_borderTRUE',
...
## write marginal effects plot dataframe to disk
save(margs_interactive_bord_gadm,
     file = here::here('Figure Data/marg_eff_bord_df_gadm_cy.RData'))

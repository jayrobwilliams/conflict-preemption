...
mod_bivar_pop <- brm(brmsformula(nl ~ pop_tot + (1 | state_ind) +
                     ...
                     future = T, file = here::here('Stanfits/pd_lm_pop_excl_cy'))
...
mod_bivar_bord <- brm(brmsformula(nl ~ border + (1 | state_ind) +
                      ...
                      iter = 4000, chains = 4, save_dso = T, save_ranef = T,
                      control = list(adapt_delta = .95), seed = 1234,
                      future = T, file = here::here('Stanfits/pd_lm_bord_excl_cy'))
...
mod_int_pop <- brm(brmsformula(nl ~ cap_dist + pop_tot + cap_dist:pop_tot +
                   ...
                   iter = 4000, chains = 4, save_dso = T, save_ranef = T,
                   control = list(adapt_delta = .95), seed = 1234,
                   future = T, file = here::here('Stanfits/pd_lm_int_pop_excl_cy'))
...
mod_int_bord <- brm(brmsformula(nl ~ cap_dist + border + cap_dist:border +
                    ...
                    iter = 4000, chains = 4, save_dso = T, save_ranef = T,
                    control = list(adapt_delta = .95), seed = 1234,
                    future = T, file = here::here('Stanfits/pd_lm_int_bord_excl_cy'))
...
mod_int_pop_controls_list <- foreach(i = 1:length(groups_list), .packages = 'brms') %dopar% {
  ...
  brm(brmsformula(nl ~ cap_dist + pop_tot + cap_dist:pop_tot + area +
      ...
      control = list(adapt_delta = .95), seed = 1234,
      file = here::here('Stanfits', paste0('pd_lm_int_pop_controls_excl_cy_', i)))
...
mod_int_bord_controls_list <- foreach(i = 1:length(groups_list), .packages = 'brms') %dopar% {
  ...
  brm(brmsformula(nl ~ cap_dist + border + cap_dist:border + area +
      ...
      iter = 4000, chains = 2, save_dso = T, save_ranef = T, cores = 2,
      control = list(adapt_delta = .95), seed = 1234,
      file = here::here('Stanfits', paste0('pd_lm_int_bord_controls_excl_cy_', i)))
...
## save combined brmsfit object
saveRDS(mod_int_bord_controls, here::here('Stanfits/pd_lm_int_bord_controls_excl_cy.rds'))

## save list of brmsfits for debugging
saveRDS(mod_int_bord_controls_list, here::here('Stanfits/pd_lm_int_bord_controls_list_excl_cy.rds'))
...
tabstr <- mcmcReg(list(mod_bivar_pop, mod_int_pop, mod_int_pop_controls),
                  ...
                  custom.model.names = paste0('Model G.', 1:3),
                  ...
                  gofnames = list(c('WAIC', '5-fold RMSE'),
                                  c('WAIC', '5-fold RMSE'),
                                  c('WAIC', '5-fold RMSE')),
                  caption = 'Linear models explaining nightlights as a function of excluded ethnic group population and capital distance. The standard deviation of the country and year random intercepts are represented by $\\sigma_\\alpha$ and $\\sigma_\\gamma$, respectively. Continuous variables logged and standarized.',
                  label = 'tab:nl_pop_excl', float.pos = 'ht!')
...
fileConn <- file(here::here('Tables/pd_pop_excl_cy.tex'))
...
tabstr <- mcmcReg(list(mod_bivar_bord, mod_int_bord, mod_int_bord_controls),
                  custom.coef.map = tab_map,
                  custom.model.names = paste0('Model G.', 4:6),
                  ...
                  gofnames = list(c('WAIC', '5-fold RMSE'),
                                  c('WAIC', '5-fold RMSE'),
                                  c('WAIC', '5-fold RMSE')),
                  caption = 'Linear models explaining nightlights as a function of excluded ethnic group border and capital distance. The standard deviation of the country and year random intercepts are represented by $\\sigma_\\alpha$ and $\\sigma_\\gamma$, respectively. Continuous variables logged and standarized.',
                  label = 'tab:nl_bord_excl', float.pos = 'ht!')
...
fileConn <- file(here::here('Tables/pd_bord_excl_cy.tex'))
writeLines(tabstr, fileConn)
close(fileConn)
...
fileConn <- file(here::here('Tables/marg_eff_excl_cy.txt'))
